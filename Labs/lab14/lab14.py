...
def num_splits(s, d):
    ...
    def helper(set1, set2, index):
        nonlocal total
        if index == len(s):
            if abs(set1 - set2) <= d:
                total += 1
        else:
            helper(set1 + s[index], set2, index + 1)
            helper(set1, set2 + s[index], index + 1)
...
class Check(object):
    ...
    def __init__(self, payable_to, money):
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skeleton_idx < len(skeleton) and code_idx == len(code):
            ...
            return edits, len(skeleton) - 1 - skeleton_idx
        if skeleton_idx == len(skeleton) and code_idx < len(code):
            edits = "".join(["+[" + c + "]" for c in code[code_idx:]])
            return edits, len(code) - 1 - code_idx
        ...
        if skel_char == code_char:
            ...
            edits = skel_char
            possibilities.append((edits + result, cost))
        # Insert
        edits = f'+[{code_char}]'
        result, cost = helper_align(skeleton_idx, code_idx + 1)
        possibilities.append((edits + result, cost + 1))
        # Delete
        edits = f'-[{skel_char}]'
...
# def my_align_skeleton(skeleton, code):
#     """
#     This implementation is my first intuition, and it's right.
#     But then i figured out that above one is a pruning version of mine, which is better.
#
#     Aligns the given skeleton with the given code, minimizing the edit distance between
#     the two. Both skeleton and code are assumed to be valid one-line strings of code.
#
...
#         # Match
#         if skel_char == code_char:
#             posses = helper_align(skeleton_idx+1, code_idx+1)
#             edits = skel_char
#             for poss in posses:
#                 possibilities.append((edits + poss[0], poss[1]))
#         # Insert
#         edits = f'+[{code_char}]'
#         posses = helper_align(skeleton_idx, code_idx+1)
#         for poss in posses:
#             possibilities.append((edits + poss[0], poss[1]+1))
#         # Delete
#         edits = f'-[{skel_char}]'
#         posses = helper_align(skeleton_idx+1, code_idx)
#         for poss in posses:
#             possibilities.append((edits + poss[0], poss[1]+1))
#         print(len(possibilities))
#         return possibilities
#
#     res = helper_align(0, 0)
#     result, cost = min(res, key=lambda x: x[1])
#
#     return result
...
def filterl(lst, pred):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldr(lst, lambda x, y: Link(x, y) if pred(x) else filterl(lst.rest, pred), Link.empty)

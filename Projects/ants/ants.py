...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        counter = 0
        while now_place is not None and now_place != beehive:
            if self.min_range <= counter <= self.max_range and len(now_place.bees) != 0:
                nearest_bee = rANTdom_else_none(now_place.bees)
                break
            else:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def find_unbuffed_ants(self):
        """
        find the un-queen-buffed ants that is behind the queen.
        """
        place2iter = self.place.exit
        ants2buff = []
        while place2iter is not None:

            # print(place2iter, place2iter.ant)
            if place2iter.ant is not None and place2iter.ant.queen_buffed is False:
                ants2buff.append(place2iter.ant)

            place2iter = place2iter.exit
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if QueenAnt.one_true_queen is not self:
            self.reduce_armor(self.armor)
        else:
            ThrowerAnt.action(self, gamestate)
            ants2buff = self.find_unbuffed_ants()
            for ant in ants2buff:
    ...
    def reduce_armor(self, amount):
        ...
        if QueenAnt.one_true_queen is self:
            bees_win()

    def remove_from(self, place):
        if self is QueenAnt.one_true_queen:
            pass # Do nothing
        else:

...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator_procedure, MacroProcedure):
            return scheme_eval(operator_procedure.apply_macro(rest, env), env)
        else:
...
def add_builtins(frame, funcs_and_names):
    ...
    # At this step, Scheme built-in functions are added to the global frame.
    # For example: global.bindings['+'] = BuiltinProcedure(scheme_add(), name=xxx)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        while cur is not nil:
            if cur.rest is nil:
                temp = scheme_eval(cur.first, env, True)
            else:
                temp = scheme_eval(cur.first, env)
...
def make_let_frame(bindings, env):
    ...
    cur = bindings
    while cur is not nil:
        validate_form(cur.first.rest, 1, 1)
        names, values = Pair(cur.first.first, names), Pair(scheme_eval(cur.first.rest.first, env), values)
        cur = cur.rest
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        raise SchemeError('Invaild macro expression')
    # END Problem 20
